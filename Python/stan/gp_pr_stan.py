...
- Gaussian Process regression with negative binomial outcome as proposed by (Chan and Vasconcelos)
- Gaussian Process regression with poisson outcome (Trangucci)
- Hierarchical Gaussian Process regression with poisson outcome (related to Trangucci)
...
Parts of the code and especially the functoin gp_pred_rng are related to code from Michael Betancourt,
that is publicly available under https://betanalpha.github.io/assets/case_studies/gp_part1/part1.html.
...
of y as an input to the function gen_pred_gp. To be more precise, y_is = log(y+1)
to account for observations that are equal to zero.
"""

gp_mod_neg_bin = """
functions {
  int[] gp_pred_rng(row_vector[] x_pred,
                     vector y_is,
                     row_vector[] x_is,
                     real alpha,
                     real length_scale,
                     real sigma) {
    int y_pred[size(x_pred)];
    int N_pred;
    int N;
    N_pred = size(x_pred);
    N = rows(y_is);

    {
      ...
      for (i in 1:N_pred)
        y_pred[i] = neg_binomial_2_rng(exp(f_pred_mu[i]), cov_f_pred[i, i] + 1e-12);
    }
    return y_pred;
  }
}
...
model {
    ...
    f_eta ~ normal(0, 1);
    sigma ~ normal(0, 1);
    y ~ neg_binomial_2(f, sigma);
}
generated quantities {
//    vector[N_pred] f_pred;
    int y_pred[N_pred];
    y_pred = gp_pred_rng(x_pred, t, x, alpha, length_scale, sigma);
//    for (n in 1:N_pred)
