...
sm = pystan.StanModel(model_code=sim_data, model_name='HLR')
...
# sample points
sample = np.random.choice(df.index, 20, replace=False)
...
gp = pystan.StanModel(model_code=gp_mod_pois, model_name='HLR')
...
plot_trace(length_scale, 'alpha', data['length_scale'])
plot_trace(sigma_fit, 'ls', data['sigma'])
# get model summary
